 ...
 * explicit IPv4 binding, bounded I/O, strict CRLF, Connection: close).
 */
#include "ui/httpd.h"
...
static int wait_ready(cbm_sock_t fd, int timeout_ms, bool write_ready) {
#ifdef _WIN32
    fd_set rfds;
    FD_ZERO(&rfds);
    FD_SET(fd, &rfds);
    ...
    int rc = select(0, write_ready ? NULL : &rfds, write_ready ? &rfds : NULL, NULL, &tv);
...
static bool socket_retryable(void) {
#ifdef _WIN32
...
static bool set_nonblocking(cbm_sock_t fd) {
#ifdef _WIN32
    u_long mode = 1;
...
static int send_all(cbm_sock_t fd, const void *data, size_t len, int64_t deadline) {
    ...
    while (off < len) {
        ...
        if (remaining <= 0)
            return -1;
        size_t chunk = len - off > INT_MAX ? INT_MAX : len - off;
#ifdef _WIN32
        int n = send(fd, p + off, (int)chunk, CBM_SEND_FLAGS);
#else
        ssize_t n = send(fd, p + off, chunk, CBM_SEND_FLAGS);
#endif
        if (n < 0 && socket_retryable()) {
            if (wait_ready(fd, (int)remaining, true) != 1)
                return -1;
            continue;
        }
        if (n <= 0)
...
cbm_httpd_t *cbm_httpd_listen_on(int port, const char *bind_address) {
    ...
    if (!set_nonblocking(fd) || bind(fd, (struct sockaddr *)&addr, sizeof(addr)) != 0 ||
        listen(fd, 16) != 0) {
        cbm_sock_close(fd);
        return NULL;
    }
...
void cbm_httpd_set_recv_deadline_ms(cbm_httpd_t *d, int ms) {
    if (d && ms > 0)
        atomic_store(&d->recv_deadline_ms, ms);
}
    ...
    *content_length = 0;
    ...
    while (p < head_stop) {
        ...
        if (header_name_is(p, nlen, "origin")) {
            ...
            if (have_authorization || !copy_header_value(colon + 1, eol, req->authorization,
                                                          sizeof(req->authorization)))
                return 400;
            have_authorization = true;
        } else if (header_name_is(p, nlen, "mcp-protocol-version")) {
            if (have_protocol || !copy_header_value(colon + 1, eol, req->protocol_version,
                                                     sizeof(req->protocol_version)) ||
                req->protocol_version[0] == '\0')
                return 400;
            have_protocol = true;
...
int cbm_httpd_read_request_checked(cbm_http_conn_t *c, cbm_http_req_t *req,
    ...
    for (;;) {
        ...
        int w = wait_ready(c->fd, (int)remaining, false);
        if (w == 0) {
            free(head);
            return 408;
        }
    ...
    if (check && (rc = check(req, clen, ctx)) != 0) {
        free(head);
        return rc;
    }
...
int cbm_httpd_read_request(cbm_http_conn_t *c, cbm_http_req_t *req) {
    return cbm_httpd_read_request_checked(c, req, NULL, NULL);
}
...
void cbm_http_reply_buf(cbm_http_conn_t *c, int status, const char *extra_headers, const void *data,
    ...
    if (c->defer_response) {
        if (len > SIZE_MAX - (size_t)hn)
            return;
        char *pending = malloc((size_t)hn + len);
        if (!pending)
            return;
        memcpy(pending, head, (size_t)hn);
        if (len > 0)
            memcpy(pending + hn, data, len);
        free(c->pending_response);
        c->pending_response = pending;
        c->pending_len = (size_t)hn + len;
        return;
    }
